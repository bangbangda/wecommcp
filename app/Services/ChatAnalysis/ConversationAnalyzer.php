...
### 1. 待办事项 (todo)
待办事项是指对话结束后仍然需要跟进的、尚未完成的工作任务。核心判断标准：这件事在对话结束时是否仍然处于"未完成"状态？

提取原则：
- owner 是需要执行动作的人，不是发起请求的人
- 只提取对话结束后仍需后续行动的事项，对话中已经完成的动作不算待办
- 必须有明确的交付物或可验证的完成标准
- 优先级判断：涉及客户投诉/线上故障/紧急上线 → high，有明确时间要求 → medium，一般事务 → low

判断流程（按顺序检查）：
1. 这件事在对话过程中是否已经完成了？→ 已完成则不提取
2. 这件事是否是一个即时性操作（加微信、发文件、拉群等）？→ 是则不提取
3. 这件事是否只是信息告知、建议或决策方案？→ 是则归入 decision，不算 todo
4. 这件事是否是对 AI 机器人发出的指令（如"帮我创建文档""帮我查一下"）？→ 是则不提取，这是人机交互不是工作任务
5. 这件事是否有明确的后续行动和交付物？→ 有则提取为 todo

应提取：
  "帮我看一下登录页面的Bug" → 需要后续排查修复，owner=对方
  "你跟进一下这个客户" → 需要持续跟进，owner=对方
  "周五前把方案发给我" → 有明确交付物和时间，owner=对方
  "我回去整理一下会议纪要" → 对话后需要完成的工作，owner=说话人

不应提取：
  "好的 我知道了""收到""OK" — 确认/回应，不是任务
  "你吃了吗""哈哈" — 非工作内容
  "加一下我微信""把你拉进群""我发你一下" — 即时性操作，对话中即可完成
  "好的，我看看" — 模糊回应，没有明确交付物
  "你可以通过这个链接注册""建议用方案B" — 信息告知/建议，不是任务分配
  "帮我创建一个文档""帮我查一下XX" — 对机器人的指令，已在对话中执行
  对话中已经完成的事（如已经发了文件、已经创建了文档、已经回答了问题）— 已完成不是待办
